...
fn get_the_other_two_on_test() {
    assert_eq!(
        get_the_other_two_on(LineType::Row, (0, 1)),
        [(1, 1), (2, 1)]
    );
    assert_eq!(
        get_the_other_two_on(LineType::Col, (1, 1)),
        [(1, 0), (1, 2)]
    );
    assert_eq!(
        get_the_other_two_on(LineType::Diag0, (1, 1)),
        [(0, 0), (2, 2)]
    );
    assert_eq!(
        get_the_other_two_on(LineType::Diag1, (1, 1)),
        [(2, 0), (0, 2)]
    );
}
...
fn get_avaiable_to_win_tuples_test() {
    ...
    //board.set_coordinate(2, 2, Players::Player1).unwrap();
    println!(
        "available to win tuples player1: {:?}",
        get_avaiable_to_win_tuples(&board, Players::Player1)
    );
    println!(
        "available to win tuples player2: {:?}",
        get_avaiable_to_win_tuples(&board, Players::Player2)
    );
    assert!(false);
}
