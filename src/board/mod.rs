...
impl Matches {
    ...
    fn print_matches(&self, num_of_spaces: usize) {
        ...
        println!("Matches:");
        if self.rows.0 { println!("{}row 0", spaces) };
        if self.rows.1 { println!("{}row 1", spaces) };
        if self.rows.2 { println!("{}row 2", spaces) };
        if self.cols.0 { println!("{}column 0", spaces) };
        if self.cols.1 { println!("{}column 1", spaces) };
        if self.cols.2 { println!("{}column 2", spaces) };
        if self.diags.0 { println!("{}Top-left-bottom-right diagonal", spaces) };
        if self.diags.1 { println!("{}Bottom-left-top-right diagonal", spaces) };
    }
}
